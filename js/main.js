...
function showClients() {
    ...
    clientIndex++;
    if (clientIndex > clients.length) { clientIndex = 1 }
    for (i = 0; i < dots.length; i++) {
        dots[i].className = dots[i].className.replace(" active", "");
    }
    clients[clientIndex - 1].style.display = "block";
    dots[clientIndex - 1].className += " active";
    setTimeout(showClients, 3000); // Change image every 3 seconds
}
...
var btnContainer = document.getElementById("filter_Wrapper");
var btns = btnContainer.getElementsByClassName("filter_link");
